...
describe('fetchMatchAnalysis ETag cache', () => {
  ...
  it('populates cache on 200 and returns data', async () => {
    (fetch as any).mockResolvedValueOnce(
      makeResponse(sample, {
        status: 200,
        headers: { ETag: 'W/"abc"', 'Cache-Control': 'public, max-age=300' },
      })
    );
  ...
  it('returns cached data on 304 and updates expiry', async () => {
    ...
    (fetch as any).mockResolvedValueOnce(
      makeResponse(sample, {
        status: 200,
        headers: { ETag: 'etag1', 'Cache-Control': 'max-age=1' },
      })
    );
  ...
  it('skips network when cache is fresh', async () => {
    ...
    (fetch as any).mockResolvedValueOnce(
      makeResponse(sample, {
        status: 200,
        headers: { ETag: 'etagX', 'Cache-Control': 'max-age=300' },
      })
    );
  ...
  it('issues conditional GET when expired', async () => {
    ...
    (fetch as any).mockResolvedValueOnce(
      makeResponse(sample, {
        status: 200,
        headers: { ETag: 'old', 'Cache-Control': 'max-age=1' },
      })
    );
    ...
    (fetch as any).mockResolvedValueOnce(
      makeResponse(sample, {
        status: 200,
        headers: { ETag: 'new', 'Cache-Control': 'max-age=300' },
      })
    );
  ...
  it('returns stale on network error if allowed', async () => {
    (fetch as any).mockResolvedValueOnce(
      makeResponse(sample, {
        status: 200,
        headers: { ETag: 'ok', 'Cache-Control': 'max-age=300' },
      })
    );
