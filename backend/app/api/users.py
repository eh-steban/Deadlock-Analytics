...
async def find_user_by_id(user_id: int, session: Annotated[AsyncSession, Depends(get_db_session)]):
...
async def find_user_by_steam_id(steam_id: str, session: Annotated[AsyncSession, Depends(get_db_session)]):
...
async def create_user(steam_id: str, session: Annotated[AsyncSession, Depends(get_db_session)]):
    try:
