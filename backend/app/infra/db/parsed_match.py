...
class ParsedMatch(SQLModel, table=True):
    ...
    players: Optional[dict] = Field(sa_column=Column(JSONB))
    damage: Optional[dict] = Field(sa_column=Column(JSONB))
    positions: Optional[dict] = Field(sa_column=Column(JSONB))
